...
export default function AutoCarousel() {
  ...
  // Criar lista duplicada para loop infinito suave
  const loopedImages = [...images, ...images]

  return (
    <div className="overflow-hidden relative max-w-full py-10 px-4">
      <motion.div
        className="flex gap-6"
        animate={{ x: ['0%', '-50%'] }} // move 50% do container para mostrar o segundo conjunto
        transition={{
          repeat: Infinity,
          repeatType: 'loop',
          duration: 20,
      ...
      >
        {loopedImages.map((src, i) => (
          <div
            key={i}
            className="flex-shrink-0 w-[45vw] md:w-[35vw] lg:w-[30vw] h-64 md:h-80 rounded-xl overflow-hidden shadow-xl relative"
          >
            <CinematicImageReveal src={src} alt={`Imagem ${i + 1}`} />
          </div>
        ))}
      </motion.div>
    </div>
